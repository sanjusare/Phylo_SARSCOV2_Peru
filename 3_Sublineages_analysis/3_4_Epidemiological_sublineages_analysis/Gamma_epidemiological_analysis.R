...
sel_lins <- list()
for (i in 1:3) {
    sel_lins[[i]] <- readLines(paste("sublineage_SubG", i, ".list",
        sep = ""))
}
...
for (i in 1:length(sel_lins)) {
    df_p$SubGs[df_p$name_analysis %in% sel_lins[[i]]] <- paste("SubG",
        i, sep = "")
}
df_p_week <- df_p %>%
...
if (length(setdiff(1:max(df_p_cg$week), unique(df_p_cg$week))) >
    ...
    coms_weeks <- data.frame(week = setdiff(1:max(df_p_cg$week),
        unique(df_p_cg$week)), SubGs = "no", count = 0, percentage = 0)
    df_p_com <- df_p_cg %>%
        dplyr::ungroup() %>%
        full_join(coms_weeks) %>%
        tidyr::complete(SubGs, week, nesting(Region), fill = list(count = 0,
            percentage = 0))
} else {
    df_p_com <- df_p_cg %>%
        dplyr::ungroup() %>%
...
rel_prev_all <- ggplot(subset(f_df), aes(x = as.POSIXct(collection_date))) +
    geom_point(data = subset(f_df, est == "No" & percentage >
        0), aes(y = percentage, color = SubGs), show.legend = F) +
    ...
    aes(y = fitted, color = SubGs)) + geom_ribbon(data = subset(f_df),
    aes(fill = SubGs, ymin = pmax(0, fitted - se_fitted), ymax = pmin(1,
...
ggsave("Relprev_Gamma_sublineages_reg.jpg", plot = rel_prev_all,
    dpi = 300, height = 5, width = 7.5)
# 5:Overall prevalence of Gamma#### data preparations
df_p_lam <- df_p_reg %>%
...
if (length(setdiff(1:max(df_p_lam_gr_tot$week), unique(df_p_lam_gr_tot$week))) >
    ...
    coms_weeks <- data.frame(week = setdiff(1:max(df_p_lam_gr_tot$week),
        unique(df_p_lam_gr_tot$week)), Gamma = "no", count = 0,
        percentage = 0)
...
ggsave("Relprev_Gamma_tot.jpg", plot = rel_prev_lam_tot, dpi = 300,
    height = 2, width = 3)
...
cas_Ls <- ggplot(subset(f_df_cas_all), aes(x = as.POSIXct(collection_date))) +
    geom_line(aes(y = smooth_lin_cases, color = SubGs)) + scale_color_manual(values = collin[names(collin) !=
    "Gamma"]) + facet_wrap(~Region) + ylab("Number of Cases") +
...
ggsave("Cases_Gamma_sublineages_reg.jpg", plot = cas_Ls, dpi = 300,
    height = 5, width = 7.5)
...
cas_tot_fin <- ggplot(subset(f_df_cas_lam_tot), aes(x = as.POSIXct(collection_date))) +
    geom_line(aes(y = totcas), color = "goldenrod") + geom_line(data = f_df_cas_lam_tot,
    aes(y = totcas), color = "goldenrod") + ylab("Cases") + scale_x_datetime(date_breaks = "2 month",
    date_labels = "%Y-%b", limits = c(as.POSIXct("2020-08-01"),
        as.POSIXct("2022-01-31"))) + theme(axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0.5),
    legend.position = "bottom", legend.title = element_blank())
ggsave("Cases_Gamma_tot.jpg", plot = cas_tot_fin, dpi = 300,
    height = 2, width = 3)
# 8:Save tables with data of prevalence and estimated cases
# by sublineage by region####
write.table(f_df_cas_all, "subGamma_epid_data.tsv", sep = "\t",
    quote = F, row.names = F)
write.table(df_p_reg, "subGamma_geno_data.tsv", sep = "\t", quote = F,
    row.names = F)
