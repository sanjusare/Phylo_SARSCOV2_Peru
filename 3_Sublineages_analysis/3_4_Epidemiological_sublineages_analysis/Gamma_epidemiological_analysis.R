...
sel_lins<-list()
for(i in 1:3){
  sel_lins[[i]]<-readLines(paste("sublineage_SubG",i,".list",sep=""))
}
...
for(i in 1:length(sel_lins)){
  df_p$SubGs[df_p$name_analysis%in%sel_lins[[i]]]<-paste("SubG",i,sep="")
}
df_p_week<-df_p%>%
...
df_p_reg<-left_join(df_p_week,reg)%>%
  tidyr::drop_na(City)
df_p_cg<-df_p_reg%>%
  dplyr::group_by(week,Region,SubGs)%>%
  dplyr::summarise(count=n())%>%
  dplyr::mutate(percentage=count/sum(count))
if(length(setdiff(1:max(df_p_cg$week),unique(df_p_cg$week)))>0){
  coms_weeks<-data.frame(week=setdiff(1:max(df_p_cg$week),unique(df_p_cg$week)),SubGs="no",count=0,percentage=0)
  df_p_com<-df_p_cg%>%
    dplyr::ungroup()%>%
    full_join(coms_weeks)%>%
    tidyr::complete(SubGs,week,nesting(Region),fill=list(count=0,percentage=0))
}else{
  df_p_com<-df_p_cg%>%
    dplyr::ungroup()%>%
    tidyr::complete(SubGs,week,nesting(Region),fill=list(count=0,percentage=0))
...
for(i in levels(as.factor(df_p_com$Region))){
  for(w in levels(as.factor(df_p_com$SubGs))){
    if(w != "no"){
      df_voc_1<-df_p_com_fin%>%
        dplyr::filter(SubGs==w&Region==i)
...
rel_prev_all<-ggplot(subset(f_df),aes(x=as.POSIXct(collection_date)))+
  geom_point(data=subset(f_df,est=="No"&percentage>0),aes(y=percentage,color=SubGs),show.legend=F)+
  ...
  new_scale_color()+
  new_scale_fill()+
  geom_line(data=subset(f_df),aes(y=fitted,color=SubGs))+
  geom_ribbon(data=subset(f_df),aes(fill=SubGs,ymin=pmax(0,fitted-se_fitted),ymax=pmin(1,fitted+se_fitted)),alpha=0.5)+
  scale_color_manual(values=collin)+
...
ggsave("Relprev_Gamma_sublineages_reg.jpg",plot=rel_prev_all,dpi=300,height=5,width=7.5)
#5:Overall prevalence of Gamma####
#data preparations
df_p_lam<-df_p_reg%>%
  dplyr::mutate(Gamma="no")
df_p_lam$Gamma[grepl("Gamma",df_p_lam$variant)]<-"yes"
df_p_lam_gr_tot<-df_p_lam%>%
  dplyr::group_by(week,Gamma)%>%
  dplyr::summarise(count=n())%>%
  dplyr::mutate(percentage=count/sum(count))
if(length(setdiff(1:max(df_p_lam_gr_tot$week),unique(df_p_lam_gr_tot$week)))>0){
  coms_weeks<-data.frame(week=setdiff(1:max(df_p_lam_gr_tot$week),unique(df_p_lam_gr_tot$week)),Gamma="no",count=0,percentage=0)
...
rel_prev_lam_tot<-ggplot(subset(f_df_lam_tot),aes(x=as.POSIXct(collection_date)))+
  ...
  theme(axis.title.x=element_blank(),
        axis.text.x=element_text(angle=90,hjust=0,vjust=0.5),
        strip.background=element_rect(fill="white"))
ggsave("Relprev_Gamma_tot.jpg",plot=rel_prev_lam_tot,dpi=300,height=2,width=3)
...
cas_Ls<-ggplot(subset(f_df_cas_all),aes(x=as.POSIXct(collection_date)))+
  ...
  theme(axis.title.x=element_blank(),
        ...
        strip.background=element_rect(fill="white"))
ggsave("Cases_Gamma_sublineages_reg.jpg",plot=cas_Ls,dpi=300,height=5,width=7.5)
#7:Estimation of total Gamma cases####
...
cas_tot_fin<-ggplot(subset(f_df_cas_lam_tot),aes(x=as.POSIXct(collection_date)))+
  ...
  ylab("Cases")+
  scale_x_datetime(date_breaks="2 month",date_labels="%Y-%b",limits=c(as.POSIXct("2020-08-01"),as.POSIXct("2022-01-31")))+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_text(angle=90,hjust=0,vjust=0.5),
        legend.position="bottom",
        legend.title=element_blank())
ggsave("Cases_Gamma_tot.jpg",plot=cas_tot_fin,dpi=300,height=2,width=3)
