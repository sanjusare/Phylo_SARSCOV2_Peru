...
sel_lins<-list()
for(i in 1:2){
  sel_lins[[i]]<-readLines(paste("sublineage_SubL",i,".list",sep=""))
}
#4:Analysis of prevalences of the sublineages by region####
#data preparations
df_p<-tabla%>%
...
for(i in 1:length(sel_lins)){
  df_p$SubLs[df_p$name_analysis%in%sel_lins[[i]]]<-paste("SubL",i,sep="")
}
df_p_week<-df_p%>%
...
if(length(setdiff(1:max(df_p_cg$week),unique(df_p_cg$week)))>0){
  coms_weeks<-data.frame(week=setdiff(1:max(df_p_cg$week),unique(df_p_cg$week)),SubLs="no",count=0,percentage=0)
  df_p_com<-df_p_cg%>%
    dplyr::ungroup()%>%
    full_join(coms_weeks)%>%
    tidyr::complete(SubLs,week,nesting(Region),fill=list(count=0,percentage=0))
}else{
  df_p_com<-df_p_cg%>%
    dplyr::ungroup()%>%
    tidyr::complete(SubLs,week,nesting(Region),fill=list(count=0,percentage=0))
...
for(i in levels(as.factor(df_p_com$Region))){
  for(w in levels(as.factor(df_p_com$SubLs))){
    if(w != "no"){
      ...
      f_df_list[[paste(i,w,sep="_")]]<-Reduce(function(x,y) dplyr::full_join(x,y,all=TRUE),list(df_voc,loessfit))%>%
...
rel_prev_all<-ggplot(subset(f_df),aes(x=as.POSIXct(collection_date)))+
  geom_point(data=subset(f_df,est=="No"&percentage>0),aes(y=percentage,color=SubLs),show.legend=F)+
  geom_errorbar(data=subset(f_df,est=="No"&percentage>0),aes(ymin=pmax(0,percentage-CI),ymax=pmin(1,percentage+CI)),color="lightgrey")+
  scale_color_manual(values=collin)+
  new_scale_color()+
  new_scale_fill()+
  geom_line(data=subset(f_df),aes(y=fitted,color=SubLs))+
  geom_ribbon(data=subset(f_df),aes(fill=SubLs,ymin=pmax(0,fitted-se_fitted),ymax=pmin(1,fitted+se_fitted)),alpha=0.5)+
  scale_color_manual(values=collin)+
...
ggsave("Relprev_Lambda_sublineages_reg.jpg",plot=rel_prev_all,dpi=300,height=5,width=7.5)
#5:Overall prevalence of Lambda####
#data preparations
df_p_lam<-df_p_reg%>%
...
df_p_lam_gr_tot<-df_p_lam%>%
  ...
  dplyr::summarise(count=n())%>%
  dplyr::mutate(percentage=count/sum(count))
if(length(setdiff(1:max(df_p_lam_gr_tot$week),unique(df_p_lam_gr_tot$week)))>0){
  coms_weeks<-data.frame(week=setdiff(1:max(df_p_lam_gr_tot$week),unique(df_p_lam_gr_tot$week)),Lambda="no",count=0,percentage=0)
  df_p_com_lam_tot<-df_p_lam_gr_tot%>%
    dplyr::ungroup()%>%
    full_join(coms_weeks)%>%
    tidyr::complete(Lambda,week,fill=list(count=0,percentage=0))
}else{
  df_p_com_lam_tot<-df_p_lam_gr_tot%>%
    dplyr::ungroup()%>%
    tidyr::complete(Lambda,week,fill=list(count=0,percentage=0))
...
rel_prev_lam_tot<-ggplot(subset(f_df_lam_tot),aes(x=as.POSIXct(collection_date)))+
  geom_point(data=subset(f_df_lam_tot,est=="No"&percentage>0),aes(y=percentage),color="forestgreen",show.legend=F)+
  ...
  geom_line(aes(y=fitted),color="forestgreen")+
  geom_ribbon(aes(ymin=pmax(0,fitted-se_fitted),ymax=pmin(1,fitted+se_fitted)),alpha=0.5,fill="forestgreen")+
  ylim(c(0,1))+
  ylab("Prevalence")+
  scale_x_datetime(date_breaks="2 month",date_labels="%Y-%b",limits=c(as.POSIXct("2020-10-01"),as.POSIXct("2022-01-31")))+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_text(angle=90,hjust=0,vjust=0.5),
        strip.background=element_rect(fill="white"))
ggsave("Relprev_Lambda_tot.jpg",plot=rel_prev_lam_tot,dpi=300,height=2,width=3)
#6:Estimation of cases by sublineage####
cas_city_cl<-cas_city%>%
  dplyr::select(City=DEPARTAMENTO,Date=FECHA_RESULTADO)%>%
  tidyr::drop_na()%>%
  dplyr::mutate(month=substr(as.character(Date),start=5,stop=6),
                year=substr(as.character(Date),start=1,stop=4),
                day=substr(as.character(Date),start=7,stop=8))%>%
...
cas_Ls<-ggplot(subset(f_df_cas_all),aes(x=as.POSIXct(collection_date)))+
  geom_line(aes(y=smooth_lin_cases,color=SubLs))+
  scale_color_manual(values=collin[names(collin)!="Lambda"])+
  facet_wrap(~Region)+
  ...
  theme(axis.title.x=element_blank(),
        ...
        strip.background=element_rect(fill="white"))
ggsave("Cases_Lambda_sublineages_reg.jpg",plot=cas_Ls,dpi=300,height=5,width=7.5)
...
cas_tot_fin<-ggplot(subset(f_df_cas_lam_tot),aes(x=as.POSIXct(collection_date)))+
  geom_line(aes(y=totcas),color="forestgreen")+
  geom_line(data=f_df_cas_lam_tot,aes(y=totcas),color="forestgreen")+
  ylab("Cases")+
  scale_x_datetime(date_breaks="2 month",date_labels="%Y-%b",limits=c(as.POSIXct("2020-10-01"),as.POSIXct("2022-01-31")))+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_text(angle=90,hjust=0,vjust=0.5),
        legend.position="bottom",
        legend.title=element_blank())
ggsave("Cases_Lambda_tot.jpg",plot=cas_tot_fin,dpi=300,height=2,width=3)
#8:Save tables with data of prevalence and estimated cases by sublineage by region####
write.table(f_df_cas_all,"subLambda_epid_data.tsv",sep="\t",quote=F,row.names=F)
write.table(df_p_reg,"subLambda_geno_data.tsv",sep="\t",quote=F,row.names=F)
