...
for (i in 1:length(files)) {
    ...
    t_list[[paste(lin, samp, sep = "_")]] <- t %>%
        dplyr::select(clock_rate = names(t)[grepl("clockRate",
            names(t))]) %>%
...
p_clock <- ggplot(t, aes(x = clock_rate)) + geom_density(alpha = 0.3) +
    labs(x = "Clock rate (s/s/y)") + theme(axis.title.y = element_blank(),
    legend.position = "bottom") + facet_wrap(~lineage)
ggsave("Clock_rate_sublineages.jpg", plot = p_clock, dpi = 500,
    height = 5, width = 5)
