...
for (i in 1:nrow(d_f1_ch)) {
    if (d_f2$value[d_f2$code == d_f1_ch$code[i]] <= 0.2 & d_f3$value[d_f3$code ==
        d_f1_ch$code[i]] <= 0.2) {
...
for (i in 1:nrow(d_f2_ch)) {
    if (d_f1$value[d_f1$code == d_f2_ch$code[i]] <= 0.2 & d_f3$value[d_f3$code ==
        d_f2_ch$code[i]] <= 0.2) {
...
for (i in 1:nrow(d_f3_ch)) {
    if (d_f1$value[d_f1$code == d_f3_ch$code[i]] <= 0.2 & d_f3$value[d_f2$code ==
        d_f3_ch$code[i]] <= 0.2) {
...
writeLines(unique(as.character(as.numeric(gsub("[A-Z]", "", d_sel)) -
    202)), "selected_Gamma_position.list")
