...
final_table <- tabla_u %>%
    dplyr::select(name_analysis, EPI_ISL = Accession.ID, date = Collection_date,
        host = Host, age = Patient.age, gender = Gender, pango_lineage = Pango.lineage,
        pango_ver = Pangolin.version, variant = Variant, Continent,
        Country, City)
write.table(final_table, "ordered_metadata.tsv", sep = "\t",
    quote = FALSE, row.names = FALSE)
tabla_rename <- tabla_u %>%
    dplyr::select(strain, name_analysis)
write.table(tabla_rename, "rename_table.tsv", sep = "\t", quote = FALSE,
