...
p_leg <- ggtree(t_grouped, aes(color = SubLsT), show.legend = F) %<+%
    ...
    "#8ad3db")) + new_scale_color() + geom_tippoint(aes(subset = label %in%
    unlist(sel_lins), color = Region)) + scale_color_manual(values = colreg) +
    geom_nodelab(aes(subset = node == as.numeric(names(sel_lins)[1])),
        color = "#58578a", nudge_x = -9e-05, nudge_y = 20) +
    geom_nodepoint(aes(subset = node == as.numeric(names(sel_lins)[1])),
        color = "#58578a", shape = 15, size = 4) + geom_nodelab(aes(subset = node ==
    as.numeric(names(sel_lins)[2])), color = "#8ad3db", nudge_x = -9e-05,
    nudge_y = 20) + geom_nodepoint(aes(subset = node == as.numeric(names(sel_lins)[2])),
    color = "#8ad3db", shape = 15, size = 4) + theme(legend.position = "bottom") +
...
# plotting the tree
p_t_fin_lam <- p_leg + theme(legend.position = "none")
ggsave("Maximum_likelihood_Lambda.jpg", plot = p_t_fin_lam, dpi = 500,
    height = 5, width = 5)
