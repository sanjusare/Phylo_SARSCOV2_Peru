...
p_leg<-ggtree(t_grouped,aes(color=SubGsT),show.legend=F)%<+%d+
  ...
  geom_nodelab(aes(subset=node==as.numeric(names(sel_lins)[1])),color="darkblue",nudge_x=-0.00009,nudge_y=20)+
  geom_nodepoint(aes(subset=node==as.numeric(names(sel_lins)[1])),color="darkblue",shape=15,size=4)+
  geom_nodelab(aes(subset=node==as.numeric(names(sel_lins)[2])),color="darkred",nudge_x=-0.00009,nudge_y=20)+
  geom_nodepoint(aes(subset=node==as.numeric(names(sel_lins)[2])),color="darkred",shape=15,size=4)+
  geom_nodelab(aes(subset=node==as.numeric(names(sel_lins)[3])),color="gold4",nudge_x=-0.00009,nudge_y=20)+
...
ggsave("Maximum_likelihood_Gamma.jpg",plot=p_t_fin_gam,dpi=500,height=5,width=5)
