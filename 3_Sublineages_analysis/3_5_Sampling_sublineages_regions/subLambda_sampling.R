...
#6:plot of correlation between estimated cases and number of genomes by week by region before and after sampling####
p_cor_lam<-ggscatter(subset(gen_group_cases,samp_count!=0),x="lam_count",y="lam_cases",
...
p_cor_sam_lam<-ggscatter(subset(gen_group_cases,samp_count!=0),x="samp_count",y="lam_cases",
                         color="black",fill="Region",shape=21,size=3,
                         add="reg.line", 
                         add.params=list(color="blue",fill="lightgray"),
                         conf.int=TRUE,cor.coef=TRUE,
...
p_sampled<-ggplot(gen_group_cases,aes(x=as.POSIXct(collection_date)))+
  ...
  geom_bar(data=subset(gen_group_cases,samp_gen>0),aes(y=samp_gen),stat="identity",color="red",fill="transparent",linewidth=0.2)+
  geom_line(aes(y=lam_cases/factor))+
  facet_wrap(~Region,nrow=3,ncol=2,scales="free_y")+
  scale_y_continuous(sec.axis=sec_axis(trans=~.*factor,name="SubL1+L2 cases"))+
...
ggsave("Lambda_sampling_distribution.jpg",plot=p_sampled,dpi=300,height=7.5,width=7.5)
