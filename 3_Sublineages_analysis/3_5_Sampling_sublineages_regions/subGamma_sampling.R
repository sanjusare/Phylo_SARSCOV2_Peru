...
# 4:plot of correlation between Gamma sublineages genomes
# and estimated cases by week by region####
p_cor_leg <- ggscatter(subset(f_df_cas_all, count != 0), x = "count",
    y = "smooth_lin_cases", color = "black", fill = "Region",
    ...
    facet_wrap(~SubGs, scales = "free") + theme(legend.position = "bottom") +
    guides(fill = guide_legend(nrow = 1))
cor_leg <- get_legend(p_cor_leg)
p_cor <- p_cor_leg + theme(legend.position = "none")
# 5:sampling by sublineage according to the number of cases
# by region by week####
sel_gens_list <- list()
...
for (i in levels(as.factor(f_df_cas_all$SubGs))) {
    ...
    f_df_cas_all_t <- f_df_cas_all %>%
        ...
        dplyr::ungroup() %>%
        dplyr::mutate(samp_gen = round(smooth_lin_cases/(sum(smooth_lin_cases,
    ...
    for (z in 1:nrow(f_df_cas_all_t)) {
        ...
        if (f_df_cas_all_t$samp_gen[z] > 0) {
            name <- paste(i, f_df_cas_all_t$Region[z], f_df_cas_all_t$collection_date[z],
                sep = "_")
            te <- df_p_reg %>%
                dplyr::mutate(collection_date = as.Date(collection_date)) %>%
...
# 6:plot of correlation between sampled Gamma sublineages
# genomes and estimated cases by week by region after
# sampling####
p_cor_sam <- ggscatter(subset(gen_group_cases, count != 0), x = "samp_count",
    y = "smooth_lin_cases", color = "black", fill = "Region",
    shape = 21, size = 3, add = "reg.line", add.params = list(color = "blue",
        fill = "lightgray"), conf.int = TRUE, cor.coef = TRUE,
    cor.coeff.args = list(method = "pearson", label.x = 1, label.sep = "\n")) +
    labs(y = "Number of Cases", x = "Number of genomes") + scale_fill_manual(values = colreg) +
    facet_wrap(~SubGs, scales = "free") + theme(legend.position = "none")
p_cor_fin <- ggarrange(p_cor, p_cor_sam, labels = c("A", "B"),
    nrow = 2, ncol = 1, common.legend = T, legend = "bottom",
    legend.grob = cor_leg)
ggsave("Gamma_sampling_correlation.jpg", plot = p_cor_fin, dpi = 500,
    height = 10, width = 7.5)
...
for (i in levels(as.factor(gen_group_cases$SubGs))) {
    ...
    p_sampled <- ggplot(d_plot, aes(x = as.POSIXct(collection_date))) +
        ...
        geom_bar(data = subset(d_plot, samp_gen > 0), aes(y = samp_gen),
            stat = "identity", color = "red", fill = "transparent",
            linewidth = 0.2) + geom_line(aes(y = smooth_lin_cases/factor)) +
        facet_wrap(~Region, nrow = 3, ncol = 2, scales = "free_y") +
        scale_y_continuous(sec.axis = sec_axis(trans = ~. * factor,
            name = paste(i, "cases"))) + scale_x_datetime(date_breaks = "1 month",
    ...
    ggsave(paste(i, "_sampling_distribution.jpg", sep = ""),
        plot = p_sampled, dpi = 300, height = 7.5, width = 7.5)
}
...
for (i in 1:length(sel_gens_list)) {
    ...
    writeLines(sel_gens_list[[i]]$name_analysis, paste(lin, "_BI_fin_v2.list",
        sep = ""))
    write.table(sel_gens_list[[i]], paste(lin, "_meta_fin_v2.tsv",
        sep = ""), sep = "\t", quote = F, row.names = F)
    write.table(d_reg, paste(lin, "_region_fin_v2.tsv", sep = ""),
