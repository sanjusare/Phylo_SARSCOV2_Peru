...
#3:read files####
#read maximum clade credibility trees
files<-args[1]
...
for(i in 1:length(filetrees)){
  name<-as.character(sapply(strsplit(filetrees[i],"_"),"[",1))
  print(name)
  trees_list[[name]]<-read.beast(filetrees[i])
  trees_text_list[[name]]<-readLines(filetrees[i])
  meta_list[[name]]<-read.csv(paste("sample_v2_",name,".tsv",sep=""),sep="\t")
}
...
pprobs<-ggplot(probs,aes(x=as.factor(sample),y=prob,fill=country))+
  geom_bar(stat="identity",position="dodge")+
  labs(x="Sample",y="Location probability of root")+
  scale_fill_manual(values=c("Argentina"="dodgerblue","Chile"="forestgreen","Peru"="firebrick"))+
...
pheights<-ggplot(heights,aes(x=as.factor(sample),y=as.POSIXct(date_median)))+
  geom_pointrange(aes(ymin=as.POSIXct(date_low95),ymax=as.POSIXct(date_hig95)),color="firebrick")+
  geom_point(aes(y=as.POSIXct(date_mean)),shape=21,fill="dodgerblue",size=5,alpha=0.5)+
  labs(y="Root Date")+
  scale_x_discrete(labels=c(1))+
  scale_y_datetime(date_breaks="1 month",date_labels="%Y-%b")+
  theme(axis.title.x=element_blank())
p_origin<-ggarrange(pheights,pprobs,ncol=1,nrow=2,labels=c("A","B"))
ggsave("Date_Location_of_Lambda_origin.jpg",plot=p_origin,dpi=500,height=7,width=4)
#5:write files of the trees depending of the country of the root####
for(i in 1:length(trees_text_list)){
  ...
  for(x in 1:length(trees_text_list[[i]])){
    if(grepl("tree STATE",trees_text_list[[i]][x])){
      tree_names_list[[as.character(x)]]<-sapply(strsplit(trees_text_list[[i]][x]," "),"[",2)
    }
  }
  ...
  for(z in 1:length(trees_list[[i]])){
    locroot<-trees_list[[i]][[z]]@data$location[trees_list[[i]][[z]]@data$node==as.character(length(trees_list[[i]][[z]]@phylo[["tip.label"]])+1)][[1]]
    if(locroot=="Peru"){
      root_per_list<-append(root_per_list,dat_trs$line[dat_trs$number==z])
    }else if(locroot=="Argentina"){
      root_arg_list<-append(root_arg_list,dat_trs$line[dat_trs$number==z])
    }else{
      root_chi_list<-append(root_chi_list,dat_trs$line[dat_trs$number==z])
    }
  }
  sel_trees_per<-trees_text_list[[i]][-as.numeric(c(root_arg_list,root_chi_list))]
  sel_trees_arg<-trees_text_list[[i]][-as.numeric(c(root_per_list,root_chi_list))]
  outfile<-paste(names(trees_text_list)[i],"_v2tree_location_f2_",sep="")
...
#7:generate tables with the first transition from and to an specific country in trees with an specific root####
...
#8:Plot the distribution of the dates where the first transition occurred####
...
ggsave("Transitions_distributions.jpg",plot=p_trans,height=7.5,width=5)
