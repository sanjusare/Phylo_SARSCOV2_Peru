...
sampling_2<-function(seed=9876345,number="19",from="2020-12-01",to="2021-09-30",genpcas=1000){
  ...
  p<-ggplot(gen_samp,aes(x=as.POSIXct(week)))+
    ...
    geom_bar(data=subset(gen_samp,samp_size>0),aes(y=samp_size),stat="identity",color="red",fill="transparent",linewidth=0.2)+
...
#5:Plot of correlations####
#before sampling
cor<-ggplot(d,aes(x=Lambda_cases,y=new_count))+
  geom_point(aes(color=Country))+
  scale_color_manual(values=count_colors[names(count_colors)%in%levels(as.factor(d$Country))])+
...
#after sampling
cor_sam<-ggplot(d,aes(x=Lambda_cases,y=samp_gen))+
  geom_point(aes(color=Country))+
  scale_color_manual(values=count_colors[names(count_colors)%in%levels(as.factor(d$Country))])+
  geom_smooth(method="lm")+
  stat_poly_eq(aes(label=after_stat(rr.label)))+
  labs(y="Lambda Genomes",x="Lambda cases")+
  theme(legend.position="none")+
  ggtitle("After sampling")
#final cor plot
p_cors<-ggarrange(cornotleg,corsam,legend="bottom",legend.grob=leg,common.legend=T)
ggsave("Lambda_correlation_plot.jpg",plot=p_cors,dpi=500,height=7.5,width=5)