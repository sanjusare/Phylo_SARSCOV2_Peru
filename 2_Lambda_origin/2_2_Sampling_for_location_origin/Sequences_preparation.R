...
#4:create the list of Lambda genomes from the selected countries to extract from GISAID####
datos_sel<-metadata%>%
  filter(grepl("Lambda",variant))%>%
  filter(Country%in%c("Argentina","Chile","Peru","Ecuador","Colombia","Mexico"))%>%
...
write.table(met_filt,"sample_initial.tsv",quote=FALSE,sep="\t",col.names=TRUE,row.names=FALSE)
writeLines(met_filt$strain,"sample_initial.list")
write.table(tabla_rename,"sample_rename_initial.tsv",sep="\t",quote=FALSE,col.names=FALSE,row.names=FALSE)
