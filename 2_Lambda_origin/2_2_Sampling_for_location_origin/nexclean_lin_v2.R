...
writeLines(tab_clean$seqName,paste("nexclean_",name,".list",sep=""))
...
write.table(data_clean,paste("final_metadata_",name,".tsv",sep=""),quote=FALSE,sep="\t",col.names=TRUE,row.names=FALSE)
data_clean_sel<-data_clean%>%
  select(name_analysis,Country)
data_clean_sel$Country[!(data_clean_sel$Country%in%c("Argentina","Chile","Peru"))]<-"Other"
write.table(data_clean_sel,paste("location_",name,".tsv",sep=""),quote=FALSE,sep="\t",col.names=FALSE,row.names=FALSE)
